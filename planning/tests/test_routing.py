...
def test_default_without_flags_is_plan_and_solve():
    decision = route_subtask("simple task")
...
def test_scenario_shapes_route_as_designed():
    ...
    assert (
        decisions["Outage bundle - Walter White (no dispatch)"]
        == PlanningMethod.PLAN_AND_SOLVE
    )

    assert (
        decisions["Faulty modem bundle - Ellen Ripley (dispatch)"]
        == PlanningMethod.LATS
    )

    assert (
        decisions["Billing bundle - Sarah Branden (credit)"]
        == PlanningMethod.LATS
    )
