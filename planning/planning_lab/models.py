...
NODE_KINDS = ("tool", "llm", "synthesis")


class Task(BaseModel):
    """A single node in the task DAG.

    Extended for the Nexlink planning agent: a node can be bound to one of
    the real MCP tools (kind="tool") so execution routes through the live
    server handlers, database and auth gate instead of a generic prompt.
    """
    ...
    def validate_tool_binding(self) -> "Task":
        if self.tool is not None and self.kind != "tool":
            raise ValueError(f"{self.id} binds tool '{self.tool}' but kind is '{self.kind}'")
        if self.kind == "tool" and not self.tool:
            raise ValueError(f"{self.id} is a tool node but no tool is bound")
        return self
